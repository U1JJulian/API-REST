...
export async function insertarFruta(req, res, next) {
  let body = "";
  ...
  req.on("end", async () => {
    try {
      ...
      if (!fruta || typeof fruta !== "string" || fruta.trim() === "") {
        return next(new Error("Esto es un mensaje de error con Manejador de Errores"));
      }
